...
```sh
make docker-build docker-push IMG=<some-registry>/clustermetrics:tag
```
...
```sh
make deploy IMG=<some-registry>/clustermetrics:tag
```
...
```sh
make build-installer IMG=<some-registry>/clustermetrics:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/clustermetrics/<tag or branch>/dist/install.yaml
```
