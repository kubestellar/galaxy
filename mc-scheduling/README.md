...
```sh
make docker-build docker-push IMG=<some-registry>/mc-scheduling:tag
```
...
```sh
make deploy IMG=<some-registry>/mc-scheduling:tag
```
...
```sh
make build-installer IMG=<some-registry>/mc-scheduling:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/mc-scheduling/<tag or branch>/dist/install.yaml
```
